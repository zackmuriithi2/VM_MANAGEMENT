...
original_author="Kyalo"
original_name="vm_management_dev"
original_urlname="VM-management-dev"
original_description="Awesome vm_management_dev created by Kyalo" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/vm_management_dev "${name}"
...
then
    ...
    echo "then run 'vm_management_dev' or 'python -m vm_management_dev'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'vm_management_dev' or 'python -m vm_management_dev' or 'poetry run vm_management_dev'"
fi
